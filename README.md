...
> From version `6.8.3`:
-   `"possible_cloud_device"`: Suggests that the device is being hosted in a cloud environment running partially or fully virtualized, often used for multi accounting and automated operation, making persistent identifiers such as `true_device_id`, `device_hash` etc. unreliable.
